...
export default function App() {
  ...
  const [deckOverrides, setDeckOverrides] = useState({
    ...
    tenor: "7 Years (T + 7Y)",
    spread: "Mid-Swap + 82 bps",
    market_date: "Market Snapshot as of 22 August 2026, 17:30 CET",
  ...
  const handleOpenPreview = async (opp) => {
    ...
    setDeckOverrides(prev => ({
      ...prev,
      market_date: `Market Snapshot as of ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}, 17:30 CET`
    }));
...
*All 10 slides in the preview canvas and PowerPoint deck are now 100% compliant.*`;
  ...
  const getSlideContent = (index, opp) => {
    ...
    const netDebtVal = deckOverrides.net_debt_str || deckOverrides.net_debt || opp.net_debt_str || (opp.net_debt ? `€${(opp.net_debt/1000).toFixed(1)}B` : "€3,192M");
    const liquidityVal = deckOverrides.liquidity_str || deckOverrides.liquidity || opp.liquidity_str || (opp.liquidity ? `€${(opp.liquidity/1000).toFixed(1)}B` : "€1,008M");
    const revenueVal = deckOverrides.revenue_str || deckOverrides.revenue || opp.revenue_str || (opp.revenue_eur_m ? `€${Number(opp.revenue_eur_m).toLocaleString()}M` : "€27,600M");
    const ebitdaVal = deckOverrides.ebitda_str || deckOverrides.ebitda || opp.ebitda_str || (opp.ebitda_eur_m ? `€${Number(opp.ebitda_eur_m).toLocaleString()}M` : "€6,226M");
    const maturityVal = deckOverrides.maturity_wall_str || deckOverrides.maturity_wall || opp.debt_maturing_24m_str || "€0M";
    const unhedgedGapVal = deckOverrides.unhedged_gap_str || deckOverrides.unhedged_gap || deckOverrides.unhedged_usd_commercial_gap || "~$6.0B - $8.0B";
    const tierVal = deckOverrides.tier || deckOverrides.rating || opp.tier || "Tier 1 (Investment Grade)";
    const rmName = deckOverrides.rm_name || opp.rm_name || "Daan Visser";
    
    // Product Family Detection
    const pType = (opp.opportunity_type || opp.type || opp.product_family || "").toLowerCase();
    const isFX = pType.includes("fx") || pType.includes("currency") || pType.includes("bunker") || clientName.includes("ASML");
    const isGreen = pType.includes("green") || pType.includes("sustainable") || pType.includes("hybrid") || clientName.includes("Enel");
    const isRates = pType.includes("rate") || pType.includes("irs") || pType.includes("swap") || clientName.includes("BASF");
    const pFamily = isFX ? "FX_HEDGE" : isGreen ? "GREEN_ESG" : isRates ? "RATES_HEDGE" : "DCM_REFI";

    const kickerText = deckOverrides.kicker || (pFamily === "FX_HEDGE" ? "FX & COMMODITY RISK ADVISORY" :
                       pFamily === "GREEN_ESG" ? "SUSTAINABLE & ESG CAPITAL STRUCTURING" :
                       pFamily === "RATES_HEDGE" ? "RATES RISK & LIABILITY MANAGEMENT" : "DCM CAPITAL STRUCTURING");

    const subtitleText = deckOverrides.subtitle || (pFamily === "FX_HEDGE" ? "Strategic FX Exposure Risk & Layered Hedging Programme" :
                         pFamily === "GREEN_ESG" ? "Inaugural Hybrid Green Bond & Sustainability Framework" :
                         pFamily === "RATES_HEDGE" ? "Pre-Hedge Swap Overlay & Rate Sensitivity Immunisation" : "Refinancing & Capital Markets Execution Framework");

    // Dynamic Scenario Variables (Slide 6)
    const fxScenUpHedged = deckOverrides.fx_scen_up_hedged || "Guaranteed Floor (1.0850)";
    const fxScenUpUnhedged = deckOverrides.fx_scen_up_unhedged || deckOverrides.unhedged_exposure || deckOverrides.unhedged_loss || deckOverrides.loss_impact || deckOverrides.unhedged_exposure_impact || "-$450M Revenue Impact";
    const fxScenSpotHedged = deckOverrides.fx_scen_spot_hedged || "1.0650 Forward Rate";
    const fxScenSpotUnhedged = deckOverrides.fx_scen_spot_unhedged || "1.0650 Spot Level";
    const fxScenDownHedged = deckOverrides.fx_scen_down_hedged || "Participate up to 1.0450";
    const fxScenDownUnhedged = deckOverrides.fx_scen_down_unhedged || deckOverrides.unhedged_gain || deckOverrides.gain_impact || "+$380M FX Gain";
        ...
        return (
          <div className="h-full flex flex-col justify-between bg-[#0C112B] text-white p-8 rounded-lg relative overflow-hidden border-l-8 border-[#FF6200]">
            ...
            <div className="border-t border-gray-800 pt-4 flex justify-between items-end text-[11px] text-gray-400">
              ...
              <div className="text-gray-400 text-[10px]">{deckOverrides.market_date || "Market Snapshot as of 24 August 2026, 17:30 CET"}</div>
            </div>
          </div>
        );
          ...
          <div className="h-full flex flex-col justify-between bg-white p-6 rounded-lg border border-gray-200">
            <div>
              <div className="flex justify-between items-start mb-3">
                <div>
                  <span className="text-[9px] font-mono text-[#FF6200] uppercase font-bold tracking-wider">Strategic Catalyst</span>
                  <h2 className="text-lg font-bold text-[#000066] mt-0.5">Executive Context & Opportunity Rationale</h2>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 text-[11px]">
                <div className="p-3 bg-orange-50/60 rounded border border-orange-200">
                  ...
                  <p className="text-gray-700 text-[10px] leading-relaxed">
                    {deckOverrides.trigger || (pFamily === "FX_HEDGE" ? "Commercial inflow shift: North American expansion increased USD revenue to >$12B against 50% hedge ratio (~$8bn gap)." : (opp.callout || opp.why_now || "Balance sheet shift identified."))}
                  </p>
                </div>
                <div className="p-3 bg-blue-50/60 rounded border border-blue-200">
                  ...
                  <p className="text-gray-700 text-[10px] leading-relaxed">
                    {deckOverrides.window || (pFamily === "FX_HEDGE" ? "EUR/USD forward points offer structural hedging pickup; volatility corridor allows zero-cost collar structuring." : "Favorable benchmark credit spreads across European issuance windows.")}
                  </p>
                </div>
                <div className="p-3 bg-emerald-50/60 rounded border border-emerald-200">
                  ...
                  <p className="text-gray-700 text-[10px] leading-relaxed">
                    {deckOverrides.action || (pFamily === "FX_HEDGE" ? "Propose staged 12M–24M layered FX hedging programme with zero-cost collar overlays to close ~$8bn gap." : (opp.action || "Propose capital structuring dialogue."))}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-12 h-full">
              <div className="col-span-5 bg-[#FF6200] p-6 text-white flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-xs font-semibold mb-2">Proactive Capital Structuring</p>
                  <p className="text-[10px] text-white/90 leading-relaxed">
                    Customized execution roadmap for {clientName} based on live market conditions and group treasury requirements.
                  </p>
                </div>
                <img src="/assets/ing_logo_white.png" alt="ING" className="h-6 object-contain self-start opacity-90" />
              </div>
              <div className="col-span-7 p-5 flex flex-col justify-between text-[11px] space-y-2">
                {[
                  { t: pFamily === "FX_HEDGE" ? "Exposure-led, not assumption-led" : "Maturity-led, not mandate-led", d: pFamily === "FX_HEDGE" ? `Addressing the ${unhedgedGapVal} USD hedge gap arising from North American revenue expansion.` : `Starting from your actual ${maturityVal} maturity profile.` },
                  { t: "Right-sized structure", d: pFamily === "FX_HEDGE" ? "Tailored combination of 12M–24M rolling FX forwards and participating collars to protect operating margin." : `Tailored combination of ${deckOverrides.notional_bond} benchmark bond and ${deckOverrides.notional_swap} overlay.` },
                  { t: "Competitive execution", d: "Live executable pricing through global financial markets FX trading and syndicate desks." },
                  { t: "Long-term partner", d: "Balance sheet underwriting strength and dedicated industry sector coverage." }
                ].map((p, i) => (
          ...
          <div className="h-full flex flex-col justify-between bg-white p-5 rounded-lg border border-gray-200">
            <div>
              <div className="flex justify-between items-start mb-2">
                <div>
                  <span className="text-[9px] font-mono text-[#FF6200] uppercase font-bold tracking-wider">Balance Sheet Foundation</span>
                  <h2 className="text-base font-bold text-[#000066] mt-0.5">Capital Structure & Liquidity Snapshot</h2>
                </div>
                <img src="/assets/ing_logo_orange.png" alt="ING" className="h-6 object-contain" />
              </div>
              <div className="grid grid-cols-4 gap-3 text-center mb-3">
                ...
                <div className="p-2.5 bg-gray-50 rounded border border-gray-200">
                  <p className="text-[10px] text-gray-500 font-semibold">{pFamily === "FX_HEDGE" ? "Unhedged FX Gap" : "24M Maturity Wall"}</p>
                  <p className="text-sm font-bold text-orange-600 mt-0.5">{pFamily === "FX_HEDGE" ? unhedgedGapVal : maturityVal}</p>
                </div>
                <div className="p-2.5 bg-gray-50 rounded border border-gray-200">
              ...
              <div className="p-3 bg-blue-50/50 rounded border border-blue-200 text-[10px] text-gray-700 space-y-1">
                <p className="font-bold text-[#000066]">Corporate Financial Standing & Treasury Policy Context</p>
                <p>• Annual Group Revenue of {revenueVal} supported by EBITDA of {ebitdaVal}.</p>
                <p>• Robust liquidity buffer of {liquidityVal} provides significant financial flexibility to support programmatic hedging structures.</p>
              </div>
            </div>
            <div className="text-center text-[9px] text-gray-400 border-t border-gray-100 pt-1.5">ING Wholesale Banking • Strictly Confidential</div>
          </div>
        ...
        return (
          <div className="h-full flex flex-col justify-between bg-white p-5 rounded-lg border border-gray-200">
            <div>
              <div className="flex justify-between items-start mb-2">
                <div>
                  <span className="text-[9px] font-mono text-[#FF6200] uppercase font-bold tracking-wider">{pFamily === "FX_HEDGE" ? "Currency Exposure Profile" : "Maturity Schedule"}</span>
                  <h2 className="text-base font-bold text-[#000066] mt-0.5">{pFamily === "FX_HEDGE" ? "FX Currency Breakdown & Hedging Gap" : "Debt Maturity Profile & Refinancing Horizon"}</h2>
                </div>
                <img src="/assets/ing_logo_orange.png" alt="ING" className="h-6 object-contain" />
              </div>
              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-6 bg-gray-50 p-3 rounded border border-gray-200 text-[10px]">
                  <p className="font-bold text-[#000066] mb-2">{pFamily === "FX_HEDGE" ? "Revenue Currency Mismatch" : "Tranche Maturity Breakdown"}</p>
                  {pFamily === "FX_HEDGE" ? (
                    <div className="space-y-1.5">
                      <div className="flex justify-between"><span>USD Exposure:</span><strong className="text-orange-700">{deckOverrides.usd_exposure || ">$12.0B (38% of total)"}</strong></div>
                      <div className="flex justify-between"><span>EUR Base Cost:</span><strong>{deckOverrides.eur_cost || "€7.5B (45% of total)"}</strong></div>
                      <div className="flex justify-between"><span>Current Hedge Ratio:</span><strong className="text-rose-600">{deckOverrides.hedge_ratio || "50% (Under-hedged)"}</strong></div>
                      <div className="flex justify-between"><span>Target Policy Ratio:</span><strong className="text-emerald-700">{deckOverrides.target_hedge_ratio || "75% - 80%"}</strong></div>
                      <div className="flex justify-between border-t pt-1 font-bold text-orange-800"><span>Unhedged Net Gap:</span><span>{unhedgedGapVal}</span></div>
                    </div>
                  ) : (
                    <div>Upcoming tranches cluster in 2026-2028.</div>
                  )}
                </div>
                <div className="col-span-6 bg-orange-50/50 p-3 rounded border border-orange-200 text-[10px] flex flex-col justify-between">
                  <div>
                    <p className="font-bold text-orange-800 mb-2">Visual FX Sizing & Policy Horizon ($B)</p>
                    <div className="space-y-2">
                      <div>
                        <div className="flex justify-between text-[9px] mb-0.5 font-medium"><span>Total USD Inflows</span><span>$12.0B</span></div>
                        <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-[#000066] h-2 rounded-full" style={{width: '100%'}}></div></div>
                      </div>
                      <div>
                        <div className="flex justify-between text-[9px] mb-0.5 font-medium"><span>Policy Target (75%)</span><span>$9.0B</span></div>
                        <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-emerald-600 h-2 rounded-full" style={{width: '75%'}}></div></div>
                      </div>
                      <div>
                        <div className="flex justify-between text-[9px] mb-0.5 font-medium"><span>Currently Hedged (50%)</span><span>$6.0B</span></div>
                        <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-[#FF6200] h-2 rounded-full" style={{width: '50%'}}></div></div>
                      </div>
                      <div>
                        <div className="flex justify-between text-[9px] mb-0.5 font-medium text-rose-700"><span>Unhedged Gap at Risk</span><span>{unhedgedGapVal}</span></div>
                        <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-rose-500 h-2 rounded-full" style={{width: '50%'}}></div></div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div>
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <h2 className="text-base font-bold text-[#000066] mt-0.5">{pFamily === "FX_HEDGE" ? "FX Scenario Analysis & Hedging Payoff" : "Refinancing Scenario Analysis"}</h2>
                </div>
                <img src="/assets/ing_logo_orange.png" alt="ING" className="h-6 object-contain" />
              </div>
              <div className="border border-gray-200 rounded overflow-hidden text-[10px]">
                <table className="w-full text-left">
                  <thead className="bg-[#000066] text-white">
                    <tr>
                      <th className="p-2">{pFamily === "FX_HEDGE" ? "Market Scenario" : "Rate Scenario"}</th>
                      <th className="p-2">{pFamily === "FX_HEDGE" ? "Layered Collar Strategy" : "Refinance Today (Locked)"}</th>
                      <th className="p-2">{pFamily === "FX_HEDGE" ? "Unhedged Exposure" : "Wait 6 Months (Unhedged)"}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200 text-gray-700">
                    <tr className="bg-orange-50/40">
                      <td className="p-2 font-medium">{pFamily === "FX_HEDGE" ? (deckOverrides.fx_scen_up_lbl || "EUR/USD +5% (USD Weakens)") : "Rates +100 bps"}</td>
                      <td className="p-2 text-emerald-700 font-bold">{pFamily === "FX_HEDGE" ? fxScenUpHedged : deckOverrides.rate_scenario_lock}</td>
                      <td className="p-2 text-rose-600 font-bold">{pFamily === "FX_HEDGE" ? fxScenUpUnhedged : deckOverrides.rate_scenario_up}</td>
                    </tr>
                    <tr className="bg-gray-50">
                      <td className="p-2 font-medium">{pFamily === "FX_HEDGE" ? (deckOverrides.fx_scen_spot_lbl || "Spot Unchanged") : "Rates Unchanged"}</td>
                      <td className="p-2 font-bold">{pFamily === "FX_HEDGE" ? fxScenSpotHedged : deckOverrides.rate_scenario_lock}</td>
                      <td className="p-2">{pFamily === "FX_HEDGE" ? fxScenSpotUnhedged : deckOverrides.rate_scenario_lock}</td>
                    </tr>
                    <tr>
                      <td className="p-2 font-medium">{pFamily === "FX_HEDGE" ? (deckOverrides.fx_scen_down_lbl || "EUR/USD -5% (USD Strengthens)") : "Rates −50 bps"}</td>
                      <td className="p-2 font-bold">{pFamily === "FX_HEDGE" ? fxScenDownHedged : deckOverrides.rate_scenario_lock}</td>
                      <td className="p-2 text-emerald-700 font-bold">{pFamily === "FX_HEDGE" ? fxScenDownUnhedged : deckOverrides.rate_scenario_down}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div>
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <h2 className="text-base font-bold text-[#000066] mt-0.5">Benchmark Yields & Market Backdrop</h2>
                </div>
                <img src="/assets/ing_logo_orange.png" alt="ING" className="h-6 object-contain" />
              </div>
              <div className="grid grid-cols-4 gap-2.5 text-center mb-2">
                <div className="p-2 bg-gray-50 rounded border border-gray-200">
                  <p className="text-[9px] text-gray-500 font-semibold">{pFamily === "FX_HEDGE" ? "EUR/USD Spot" : "5Y EUR Swap"}</p>
                  <p className="text-xs font-bold text-[#000066] mt-0.5">{pFamily === "FX_HEDGE" ? (deckOverrides.spot_fx || "1.0650") : (deckOverrides.swap_5y || "2.62%")}</p>
                </div>
                <div className="p-2 bg-gray-50 rounded border border-gray-200">
                  <p className="text-[9px] text-gray-500 font-semibold">{pFamily === "FX_HEDGE" ? "12M Forward Pts" : "10Y Bund"}</p>
                  <p className="text-xs font-bold text-gray-900 mt-0.5">{pFamily === "FX_HEDGE" ? (deckOverrides.forward_points || "+185 pts") : (deckOverrides.bund_10y || "2.61%")}</p>
                </div>
                <div className="p-2 bg-gray-50 rounded border border-gray-200">
                  <p className="text-[9px] text-gray-500 font-semibold">{pFamily === "FX_HEDGE" ? "ECB Refi Rate" : "iBoxx BBB Spread"}</p>
                  <p className="text-xs font-bold text-orange-600 mt-0.5">{pFamily === "FX_HEDGE" ? (deckOverrides.ecb_rate || "2.25%") : (deckOverrides.iboxx_bbb || "115 bps")}</p>
                </div>
                <div className="p-2 bg-gray-50 rounded border border-gray-200">
                  <p className="text-[9px] text-gray-500 font-semibold">{pFamily === "FX_HEDGE" ? "Fed Funds Target" : "iTraxx Main"}</p>
                  <p className="text-xs font-bold text-emerald-700 mt-0.5">{pFamily === "FX_HEDGE" ? (deckOverrides.fed_rate || "4.00–4.25%") : (deckOverrides.itraxx_main || "58 bps")}</p>
                </div>
              </div>
              <div className="p-3 bg-blue-50/40 rounded border border-blue-100 text-[10px] text-gray-700 space-y-1">
                <p className="font-bold text-[#000066]">Central Bank Policy & Cross-Currency Backdrop</p>
                <p>• ECB Refinancing Rate at {deckOverrides.ecb_rate || "2.25%"}; Fed Funds Target at {deckOverrides.fed_rate || "4.00–4.25%"}.</p>
                <p>• {pFamily === "FX_HEDGE" ? "Favorable EUR/USD forward point differentials (+185 pts) support programmatic multi-tenor hedging." : "Tightening European investment grade credit spreads support attractive new-issue concession pricing."}</p>
              </div>
            </div>
            <div className="text-center text-[9px] text-gray-400 border-t border-gray-100 pt-1.5">ING Wholesale Banking • Strictly Confidential</div>
          </div>
            ...
            <div>
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <h2 className="text-base font-bold text-[#000066] mt-0.5">{pFamily === "FX_HEDGE" ? "Indicative FX Risk Management Term Sheet" : "Indicative Debt Financing Term Sheet"}</h2>
                </div>
                <img src="/assets/ing_logo_orange.png" alt="ING" className="h-6 object-contain" />
              </div>
              <div className="border border-gray-200 rounded overflow-hidden text-[10px]">
                <table className="w-full">
                  <tbody className="divide-y divide-gray-200">
                    ...
                    <tr><td className="p-1.5 font-bold">Instrument Format</td><td className="p-1.5">{deckOverrides.term_format || (pFamily === "FX_HEDGE" ? "Layered FX Forwards & Participating Collars" : "Senior Unsecured Euro Medium Term Note (EMTN)")}</td></tr>
                    <tr className="bg-gray-50"><td className="p-1.5 font-bold">Notional Sizing</td><td className="p-1.5">{deckOverrides.term_notional || deckOverrides.notional_bond || (pFamily === "FX_HEDGE" ? "$1.5B - $3.0B Staged Programme" : "EUR 600,000,000")}</td></tr>
                    <tr><td className="p-1.5 font-bold">Tenor / Maturity</td><td className="p-1.5">{deckOverrides.term_tenor || deckOverrides.tenor || (pFamily === "FX_HEDGE" ? "12 to 24 Months Rolling Quarterly" : "7 Years (T + 7Y)")}</td></tr>
                    <tr className="bg-gray-50"><td className="p-1.5 font-bold">Indicative Pricing</td><td className="p-1.5 font-bold text-orange-700">{deckOverrides.term_pricing || deckOverrides.spread || (pFamily === "FX_HEDGE" ? "Zero Net Upfront Premium (Floor: 1.0850 / Cap: 1.0450)" : "Mid-Swap + 82 bps")}</td></tr>
            ...
            <div>
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <h2 className="text-base font-bold text-[#000066] mt-0.5">{pFamily === "FX_HEDGE" ? "FX Execution & Layered Roll Framework" : "Roadmap & Syndicate Timeline"}</h2>
                </div>
                <img src="/assets/ing_logo_orange.png" alt="ING" className="h-6 object-contain" />
              </div>
              <div className="grid grid-cols-4 gap-2 text-[10px] text-center">
                {(pFamily === "FX_HEDGE" ? [
                  ...
                  "T - 4 Weeks: Documentation: Confirm base prospectus & swap schedules",
                  "T - 2 Weeks: Pre-Hedge Execution: Execute treasury pre-hedge overlay",
                  "T - 1 Week: Global Roadshow: Syndicate investor bookbuilding meetings",
                  "T-Day: Pricing & Settlement: Final pricing, allocation & closing"
                ]).map((st, idx) => (
            ...
            <div>
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <h2 className="text-base font-bold text-[#000066] mt-0.5">Regulatory Notices & Target Market Classification</h2>
                </div>
                <img src="/assets/ing_logo_orange.png" alt="ING" className="h-6 object-contain" />
              </div>
  ...
  const slideTitles = [
    "01. Cover Slide", "02. Agenda", "03. Executive Summary", 
    "04. Debt Maturity Profile", "05. Market Backdrop", "06. Refinancing Rationale", 
    "07. Proposal Term Sheet", "08. Why Execute With Us", "09. Deal Team & Timeline", "10. Disclaimers"
  ];
          ...
          <div className="lg:col-span-8">
            <div className="mb-6">
              ...
              <h1 className="text-3xl font-serif font-bold text-[#0C112B] mb-2">
                {opportunities.length} opportunities surfaced from database
              </h1>
              <p className="text-sm text-gray-600 leading-relaxed">
                Each match combines live market rates curves and internal corporate debt schedules from PostgreSQL into pre-drafted pitchbooks.
              </p>
            </div>
                ...
                opportunities.map((opp) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs text-gray-500 italic mb-4">{opp.subtitle}</p>

                      <div className="flex flex-wrap gap-2 mb-4">
                        {opp.chips && opp.chips.map((chip, cIdx) => (
                          <span
                            key={cIdx}
                            className="bg-[#F1F3F5] text-gray-700 text-xs px-3 py-1 rounded-md font-medium"
                          >
                            {chip}
                          </span>
                        ))}
                      ...
                      <div className="border-l-4 border-[#FF6200] bg-gray-50 p-3.5 rounded-r-md text-xs text-gray-700 leading-relaxed mb-5">
                        {opp.callout}
                      </div>

                      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                        <div className="text-xs text-gray-500">
                          <span className="font-semibold text-gray-700">Draft ready</span> · {opp.slides_count || 10} slides pre-populated from relational DB
                        </div>
              ...
              <div className="flex-1 p-6 overflow-y-auto space-y-4 text-xs">
                ...
                {ingestTab === "rss" && (
                  <div className="space-y-3">
                    <p className="text-gray-600 font-medium">
                      Live market news fetched for <b>{ingestClient.name}</b>. Ingesting any article extracts triggers via Gemini and recalculates opportunity rankings:
                    </p>
                    {rssLoading ? (
                ...
                {ingestTab === "preset" && (
                  <div className="space-y-4">
                    ...
                    >
                      {ingestingAction ? "Extracting Signals with Gemini Flash..." : "Ingest Touchpoint & Update Signals"}
                    </button>
                  </div>
                )}
              </div>

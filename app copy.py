...
def generate_pitchbook():
    ...
    (CORE-01 to CORE-11 + DEBT/GREEN/FX product suites).
    """
    try:
        from pptx import Presentation
        from pptx.util import Inches, Pt
        ...
        # ---------------------------------------------------------------------
        # 1. Extract Input Payload & Context
        # ---------------------------------------------------------------------
        client_id = normalize_text(data.get("client_id", "CLI009_ENEL"))
        # Normalize client_id for database lookup
        client_id = normalize_client_id(client_id)
        
        client_name = normalize_text(data.get("client_name", ""))
        title = normalize_text(data.get("title", "Financial Markets Strategy"))
        product = normalize_text(data.get("product", "Strategic Financing"))
        ...
        if not client_name or client_name == "Corporate Client":
            client_name = "Enel SpA" if "ENEL" in client_id.upper() else client_id

        tier = "Tier 1"
        hq_country = "Europe"
        revenue_eur_m = "€92,800M" if "ENEL" in client_id.upper() else "N/A"
        rm_name = "Senior Relationship Manager"

        # ---------------------------------------------------------------------
        # 2. Database Lookup (ca.client_master)
        # ---------------------------------------------------------------------
        try:
            conn = get_db_connection()
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT client_name, tier, hq_country, revenue_eur_m, rm_name FROM ca.client_master WHERE client_id = %s;",
                    (client_id,)
                )
                row = cur.fetchone()
                if row:
                    ...
                    if row[3]:
                        revenue_eur_m = f"€{row[3]:,.0f}M"
                    rm_name = row[4] or rm_name
            conn.close()
        except Exception as db_err:
            logger.warning("ca.client_master lookup fallback applied: %s", db_err)
            if "ENEL" in client_id.upper():
                revenue_eur_m = "€92,800M"
                hq_country = "Rome, Italy"

        # ---------------------------------------------------------------------
        # 3. Design System & Palette Setup
        # ---------------------------------------------------------------------
        ING_ORANGE = RGBColor(255, 98, 0)       # #FF6200
        ING_NAVY = RGBColor(0, 0, 102)          # #000066
        ING_DARK_SLATE = RGBColor(12, 17, 43)   # #0C112B (Cover Background)
        ING_WHITE = RGBColor(255, 255, 255)
        ING_CARD_BG = RGBColor(245, 247, 250)   # #F5F7FA Container Fill
        ...
        # ---------------------------------------------------------------------
        # 4. Slide Shell Helpers (Logo & Headers)
        # ---------------------------------------------------------------------
        def add_logo(slide, is_white=False):
            """
            Embeds the official ING Lion logo using dynamic path resolution.
            Preserves native aspect ratio (width=0.85 in) with a fail-safe styled wordmark fallback.
            """
            # 1. Dynamically locate the directory where the executing script lives
            base_dir = os.path.dirname(os.path.abspath(__file__))

            # 2. Select primary and fallback logo targets
            ...
            # 3. Dynamic candidate paths (both absolute relative to script and working directory)
            candidates = [
            ...
            for path in candidates:
                if path and os.path.exists(path):
                    try:
                        # Specify width=0.85 in; python-pptx auto-computes height to lock native aspect ratio
                        ...
                        logger.warning("Could not load image from %s: %s", path, e)
                        continue

            # Fallback Wordmark Container if asset files are unavailable
            try:
                ...
                logger.error("Failed to render logo text fallback: %s", e)

        def add_header_footer(slide, slide_id: str, title: str, subtitle: str, data_source: str):
            """Standard header banner, category chip, title, subtitle, and bottom data caption."""
            # Top Orange Indicator Strip
            ...
            # Data Source / Confidential Footer
        ...
        # --- CORE-04: COMPANY OVERVIEW ---
        slide4 = prs.slides.add_slide(blank_layout)
        add_header_footer(slide4, "CORE-04", "Company Overview", "Operational scale, franchise footprint, and core enterprise metrics", "PB_Company_Description, Cand5_Client_Master")

        stats = [
            ("REPORTED REVENUE", revenue_eur_m, "Latest fiscal reported scale"),
        ...
        for i, (s_t, s_v, s_s) in enumerate(stats):
            box = slide4.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8 + (i * 4.0)), Inches(1.4), Inches(3.7), Inches(1.3))
            box.fill.solid()
        ...
        ov_box = slide4.shapes.add_textbox(Inches(0.8), Inches(3.0), Inches(11.7), Inches(3.6))
        tf_o = ov_box.text_frame
        tf_o.word_wrap = True

        sections = [
            ("Business Model & Global Positioning", f"{client_name} maintains an established position across core regulated and market segments with strong balance sheet sponsor support."),
            ("Strategic Revenue Quality & Defensive Moat", "Diversified contracted cashflows, disciplined leverage management, and proactive ESG/transition investments."),
            ("Treasury & Funding Architecture", "Group Treasury manages centralized funding, hybrid capital, and active derivatives pre-hedging programs."),
        ]
        for h, b in sections:
        ...
        # =====================================================================
        # 6. PRODUCT-SPECIFIC SLIDES (DEBT / GREEN / FX)
        # =====================================================================
        if "DEBT" in catalog_family.upper() or "CAPITAL" in catalog_family.upper() or "CREDIT" in catalog_family.upper():
            # --- DEBT-01: CREDIT & FUNDING PROFILE ---
            slide_d1 = prs.slides.add_slide(blank_layout)
            add_header_footer(
                slide_d1,
                "DEBT-01",
                "Credit & Funding Profile",
                "Secondary spread positioning, rating curve benchmarks, and refinancing economics",
                "Ext_Credit_Spreads"
            )

            # Left Column: Spread Commentary
            tb_d1 = slide_d1.shapes.add_textbox(Inches(0.8), Inches(1.4), Inches(7.5), Inches(5.2))
            ...
            debt_items = [
                f"• Secondary Spreads: {client_name}'s senior curve trades tight (~75-85 bps over Mid-Swaps) against the European Utility BBB+ composite curve.",
                "• Legacy Coupon Gap: Outstanding 2026/2027 maturities carry a ~1.20% coupon, creating a +330 bps refinancing step-up at prevailing yield levels.",
                "• Pre-Hedging Recommendation: Execute forward-starting interest rate swaps (IRS) or swaptions to lock in current rate levels prior to transaction execution.",
            ]
            ...
            for i, (rc_title, rc_val, rc_sub) in enumerate(rate_cards):
                card = slide_d1.shapes.add_shape(
            ...
            # --- DEBT-04: INDICATIVE REFINANCING ECONOMICS TABLE ---
            slide_d4 = prs.slides.add_slide(blank_layout)
            add_header_footer(slide_d4, "DEBT-04", "Indicative Refinancing Economics", "Baseline vs. stressed market scenarios and illustrative annual P&L impact", "DT_Simulation, Mkt_Rates_Curves")

            table_shape = slide_d4.shapes.add_table(4, 4, Inches(0.8), Inches(1.6), Inches(11.7), Inches(3.0))
            tbl = table_shape.table
            headers_d = ["Refinancing Tranche Scenario", "Baseline Refi Rate", "Stressed Rate (+50 bps)", "Illustrative Annual P&L Impact"]
            for col_idx, h_text in enumerate(headers_d):
            ...
            rows_d = [
                ["5-Year Senior Benchmark (€1.0B)", "3.45% All-in", "3.95% Stressed", "-€5.0M / year"],
                ["8-Year Green / Sustainability Tranche (€750M)", "3.85% All-in", "4.35% Stressed", "-€3.75M / year"],
                ["10-Year Subordinated Hybrid (€1.25B)", "4.65% All-in", "5.15% Stressed", "-€6.25M / year"],
            ]
            for r_idx, row in enumerate(rows_d):
                for c_idx, val in enumerate(row):
                    cell = tbl.cell(r_idx + 1, c_idx)
                    cell.text = val
                    cell.fill.solid()
                    cell.fill.fore_color.rgb = ING_CARD_BG if r_idx % 2 == 0 else ING_WHITE
                    p = cell.text_frame.paragraphs[0]
                    p.font.size = Pt(10)
                    p.font.color.rgb = ING_TEXT_DARK
                    if c_idx > 0:
        ...
        elif "SUSTAIN" in catalog_family.upper() or "ESG" in catalog_family.upper():
            # --- GREEN-01: SUSTAINABILITY FRAMEWORK ---
            slide_g1 = prs.slides.add_slide(blank_layout)
            add_header_footer(slide_g1, "GREEN-01", "Sustainability Framework & Eligibility", "Use-of-proceeds taxonomy, framework maturity, and SPT alignment", "CA_Sustainability, Ctx_House_Views")
            tbg1 = slide_g1.shapes.add_textbox(Inches(0.8), Inches(1.5), Inches(11.7), Inches(5.0))
            tfg1 = tbg1.text_frame
            p = tfg1.paragraphs[0]
            p.text = "Framework Alignment & Greenium Opportunity"
            p.font.bold = True
            p.font.size = Pt(14)
            p.font.color.rgb = GREEN_COLOR
            for gp in [
                "• Established Green Financing Framework aligned with ICMA Green Bond Principles.",
                "• Eligible Categories: Renewable energy capacity, grid modernization, and energy efficiency programs.",
                "• Pricing Advantage: Greenium differential captures 3 to 7 bps pricing concession from dedicated ESG funds."
            ]:
                p = tfg1.add_paragraph()
                p.text = gp
                p.font.size = Pt(11)
                p.space_before = Pt(6)

        # =====================================================================
        # 7. CLOSING MODULES (CORE-09, CORE-10, CORE-11)
        # =====================================================================

        # --- CORE-09: WHY ING ---
        slide9 = prs.slides.add_slide(blank_layout)
        add_header_footer(slide9, "CORE-09", "Why ING", "Sector leadership, transaction track record, and dedicated coverage team", "CA_Ext_Deals")

        tb9 = slide9.shapes.add_textbox(Inches(0.8), Inches(1.4), Inches(7.5), Inches(5.2))
        tf9 = tb9.text_frame
        tf9.word_wrap = True
        ...
        # Right: Team Card
        tcard = slide9.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(8.6), Inches(1.4), Inches(3.9), Inches(5.1))
        tcard.fill.solid()
        ...
        members = [
            (rm_name, "Relationship Manager, Global Coverage"),
            ("Managing Director", "Head of Debt Capital Markets"),
            ("Director", "Financial Markets Derivatives & Hedging"),
        ...
        # --- CORE-10: PROPOSED NEXT STEPS ---
        slide10 = prs.slides.add_slide(blank_layout)
        add_header_footer(slide10, "CORE-10", "Proposed Next Steps", "Implementation roadmap, validation milestones, and key contacts", "PB_Selected_Opportunity")
        ...
        for i, (st_t, st_items) in enumerate(steps_data):
            c_box = slide10.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8 + (i * 4.0)), Inches(1.5), col_w, Inches(4.8))
            c_box.fill.solid()
        ...
        # --- CORE-11: IMPORTANT INFORMATION (DISCLAIMER) ---
        slide11 = prs.slides.add_slide(blank_layout)
        add_header_footer(slide11, "CORE-11", "Important Information", "Regulatory classification, non-research declaration, and disclaimers", "Ref_Service_Catalogue, Meta_Provenance")

        tb11 = slide11.shapes.add_textbox(Inches(0.8), Inches(1.4), Inches(11.7), Inches(5.2))
        ...
        # ---------------------------------------------------------------------
        # 8. Deliver Binary Buffer via send_file
        # ---------------------------------------------------------------------
        out = io.BytesIO()
        prs.save(out)
        out.seek(0)

        logger.info("Master Pitchbook generated successfully for client=%s, title=%s", client_name, title)

        return send_file(
            out,
            mimetype="application/vnd.openxmlformats-officedocument.presentationml.presentation",
            as_attachment=True,
            download_name=f"ING_{client_id}_Pitchbook.pptx",
        )

...
with tab2:
    ...
    if st.button(
        ...
        with st.spinner(
            ...
            try:
                ...
                # Reset downstream states so new opportunities start fresh
                st.session_state.pop("compliance_result", None)
                st.session_state.pop(f"compliance_bullets_{client_id}", None)
                st.session_state.pop(f"pitchbook_bytes_{client_id}", None)
    ...
    if opp:
        ...
        evidence_record_count = opp.get(
            "evidence_record_count",
            0,
        )

        retrieval_mode = opp.get(
            "retrieval_mode",
            "N/A",
        )

        evidence_sources = opp.get(
            "evidence_sources",
            [],
        )
        ...
        if evidence_sources:
            ...
            with st.expander(
                f"📚 Evidence Sources ({source_count})"
            ):
                ...
                for source in evidence_sources:
                    ...
                    if isinstance(
                        ...
                        # Plain text display instead of HTML
        ...
        # ---------------------------------------------------------------------
        # Opportunity rationale - UPDATED: Plain text with Streamlit components
        # ---------------------------------------------------------------------
        ...
        validation_gap = opp.get(
            "validation_gap",
            "N/A",
        )

        # Display rationale using Streamlit components instead of HTML
        st.markdown("---")
        st.markdown("#### 📋 Deal Rationale & Next Best Action")
        
        # Use info box for rationale
        st.info(rationale)
        
        # Display trigger, urgency, validation gap as plain text
        st.markdown(f"**Trigger Origin:** `{trigger}`")
        
        # Urgency with appropriate color
        if urgency == "High":
        ...
        # =====================================================================
        # Secondary Opportunities - UPDATED: Plain text with Streamlit components
        # =====================================================================

        st.markdown("### 🔄 Cross-Asset & Conditional Opportunity Discovery")
        ...
        if secs:
            ...
            for idx, secondary in enumerate(secs):
                ...
                family = secondary.get(
                    "catalog_family",
                    "N/A",
                )

                product = secondary.get(
                    "product",
                    "N/A",
                )

                condition = secondary.get(
                    "condition",
                    "N/A",
                )

                # Display each secondary opportunity with Streamlit components
                with st.container():
            ...
            st.info(
                "No secondary or conditional opportunities were identified."
            )
        ...
        st.info(
            "Run opportunity discovery after ingesting one or more source signals."
        )

# =============================================================================
# TAB 3
# Regulatory Compliance Gateway - ENHANCED (FIXED)
# =============================================================================
    ...
    if not opp:
        ...
        if result:
            ...
            if suggested_edits:
                ...
                for key, edit in suggested_edits.items():
            ...
            # -------- Apply Suggested Changes to Pitchbook --------
            st.markdown("---")
            ...
            with col1:
                if st.button(
                    ...
                    if suggested_edits:
                        current_bullets = st.session_state[bullets_key]
                        updated_bullets = list(current_bullets)
                            ...
                            if isinstance(edit, dict):
                                ...
                                if original and replacement:
                                    for i, bullet in enumerate(updated_bullets):
                                        if original in bullet:
                                            updated_bullets[i] = bullet.replace(original, replacement)

                        st.session_state[bullets_key] = updated_bullets
                        st.success("✅ Suggested edits applied to pitchbook bullets!")
                        st.rerun()
                    else:
                        st.info("No suggested edits available.")

            with col2:
                # Button to copy bullets to Tab 4
                if st.button(
                    "📋 Use These Bullets for Pitchbook",
                    key=f"btn_use_bullets_{client_id}",
                ):
                    st.session_state[f"pitchbook_bullets_{client_id}"] = st.session_state[bullets_key]

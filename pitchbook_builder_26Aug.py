...
def build_pitchbook(ctx, opp, compliance_bullets=None, overrides=None):
    ...
    add_header(s2, "Executive Context & Opportunity Rationale", category="STRATEGIC CATALYST")
    add_logo(s2)
    add_footer(s2)
    ...
    add_header(s4, "Capital Structure & Treasury Health Profile", category="BALANCE SHEET FOUNDATION")
    add_logo(s4)
    add_footer(s4)
    ...
    add_header(s7, "Benchmark Yields & Market Backdrop", category="MARKET INTELLIGENCE")
    add_logo(s7)
    add_footer(s7)
    ...
    add_header(s10, "Regulatory Notices & Target Market Classification", category="REGULATORY DISCLOSURES")
    add_logo(s10)
    add_footer(s10)
